...
class Bootstrap_NavWalker extends Walker_Nav_Menu
{
    ...
    public function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0)
    {
        ...
        if ($item->object == 'menu_asset') {
            $acf = get_fields($item->object_id);

            $template = '<div class="menu-asset dropdown-item">
                <div class="menu-asset-body">
                    <span class="menu-asset-heading">' . $acf['heading'] . '</span>
                    <span class="menu-asset-title">' . $title . '</span>
                    <a href="' . $acf['link'] . '" class="btn btn-sm">' . $acf['button_text'] . '</a>
                </div>
                <div class="img-background" style="background-image: url(' . $acf['image'] . ')"></div>
            </div>';

            $item_output = $args->before;
            $item_output .= $args->link_before . $template . $args->link_after;
            ...
            if ($item->description) {
                $item_output .= '<span class="dropdown-item-description">' . $item->description .'</span>';
            }

            $item_output .= '</a>';

            if ($depth === 0 && in_array('menu-item-has-children', $classes)) {
                $item_output .= '<button class="button-unstyled dropdown-toggle" data-toggle="dropdown" 
id="navbar-dropdown-menu-link-' . $item->ID . '"><span></span><span></span></button>';
            }
