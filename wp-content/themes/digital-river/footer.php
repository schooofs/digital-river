...
<script>
  $(function() {
    ...
    function addExpandedSpeaker($speaker, insertIndex) {
      if (closing) {
        setTimeout(function(){
          ...
          $speaker.find('.speaker-bio').clone().insertAfter('#speakers .speaker:eq('+insertIndex+')').slideDown(animationTime);
        }, animationTime + 100);
      } else {
        $speaker.addClass('expanded');
        $speaker.find('.speaker-bio').clone().insertAfter('#speakers .speaker:eq('+insertIndex+')').slideDown(animationTime);
    ...
    $('#speakers').on('click', '.speaker:not(.expanded) .speaker-info button', function() {
      ...
      var perRow = (window.matchMedia('(max-width: 450px)').matches) ? 1 : (window.matchMedia('(max-width: 666px)').matches) ? 2 : (window.matchMedia('(max-width: 991px)').matches) ? 3 : 4;
      var row = Math.floor(index/perRow);
      var insertIndex = (row * perRow) + (perRow - 1);
      if (!$('#speakers .speaker:eq('+insertIndex+')').length) insertIndex = $('.speaker').length - 1;
      // console.log('--------------');
